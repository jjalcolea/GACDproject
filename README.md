...
The **run_analysis.R** file is an R Script that, when executed, proccesses the raw data and generates the file **tidyDataSet.txt**. It has been developed and tested in R version 3.0.2 (2013-09-25) under [Ubuntu Linux](http://www.ubuntu.com/download) 14.04 (64 bits). The script assumes that the full file structure and contents of the repository is present (i.e. the "UCI HAR Dataset" directory and its contents). Check the **CodeBook.md** file for a high level description of the logic contained in the script.
